...
//get test details of specific patient according to test request table
/*
this.getTestDetailOfPatient= function(id){
	return new Promise((resolve, reject) => {
		testReqSchema.find({HIN: id}).exec().then(data => {
							resolve({'status': 200, 'message':'get single data', 'data': data});
		}).catch(err => {
							reject({'status': 404, 'message':'err:-'+err});
					})
...
this.getRecentPatientTests = function(id){
	return new Promise((resolve, reject) => {
		testReqSchema.find({HIN: id}).exec().then(data => {
							resolve({'status': 200, 'message':'get recent test data', 'data': data});
		}).catch(err => {
							reject({'status': 404, 'message':'err:-'+err});
					})
	})
...
//get one test request according to test request
this.getSpecificTestRequest = function(id){
	return new Promise((resolve, reject) => {
		testReqSchema.find({id: id}).exec().then(data => {
							resolve({'status': 200, 'message':'get single data', 'data': data});
		}).catch(err => {
							reject({'status': 404, 'message':'err:-'+err});
					})
...
this.updatePatientTestResults = function(id, updateData) {
	return new Promise((resolve, reject) => {
		patientSchema.update({HIN: id}, updateData).then(() => {
							resolve({'status': 200, 'message':'update Patient test results'});
		}).catch(err => {
							reject({'status': 404, 'message':'err:-'+err});
					})
	})
...
this.updateStatus = function(id, updateData) {
	return new Promise((resolve, reject) => {
		testReqSchema.update({HIN: id}, updateData).then(() => {
							resolve({'status': 200, 'message':'update test request status'});
		}).catch(err => {
							reject({'status': 404, 'message':'err:-'+err});
					})
	})
